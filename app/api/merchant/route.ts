...
export async function GET() {
  ...
  return NextResponse.json({ metrics: { ai_assisted_sessions: sessions.size, products_discovered: products.data?.length || 0, ai_recommendations: recommendations, carts_created: carts.data?.length || 0, orders: orderRows.length, revenue: paid.reduce((sum, order) => sum + Number(order.amount || 0), 0), conversion_rate: sessions.size ? Math.round((paid.length / sessions.size) * 100) : 0 }, orders: orderRows.slice(0, 10), activity: audit.data || [] });
}
