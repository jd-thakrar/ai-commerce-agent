...
export default function MerchantPage() {
  ...
  async function deactivateCampaign() {
    setActivating(true);
    const response = await fetch('/api/campaigns/deactivate', { method: 'POST' });
    const result = await response.json();
    if (!response.ok) setError(result.error || 'Unable to deactivate campaign.');
    else {
      const refreshed = await fetch('/api/merchant');
      setData(await refreshed.json());
    }
  ...
  const campaignActive = data?.active_campaign?.status === 'active';
  const campaignPanel = <section className="mt-8 flex flex-col justify-between gap-5 border border-black/10 bg-white p-5 md:flex-row md:items-center"><div><p className="text-xs font-medium tracking-widest text-black/40">CAMPAIGNS</p><h2 className="mt-2 text-lg font-semibold">Weekend Laptop Sale</h2><p className="mt-1 text-sm text-black/50">10% off Laptop products · {campaignActive ? 'Active now' : 'Ready to launch'}</p></div><div className="flex items-center gap-5"><div><p className="text-xs text-black/45">Campaign-influenced orders</p><p className="mt-1 text-xl font-semibold">{metrics?.campaign_influenced_orders || 0}</p></div><button onClick={campaignActive ? deactivateCampaign : activateCampaign} disabled={activating} className="rounded-full bg-black px-4 py-2 text-sm text-white disabled:opacity-40">{activating ? 'Updating...' : campaignActive ? 'Deactivate campaign' : 'Launch campaign'}</button></div></section>;
